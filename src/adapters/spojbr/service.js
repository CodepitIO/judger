        ...
        [
          ...
          (next) => {
            browser
              ...
              .then(() => browser.check("autologin"))
              .then(() => browser.pressButton("submit", next));
          },
        ],
        (err) => {
    ...
    function send(sub, retry, callback) {
      async.waterfall(
        [
          ...
          (next) => {
            browser
              .fill('input[name="problemcode"]', sub.problemId)
              .then(() => browser.select('select[name="lang"]', sub.language))
              .then(() => browser.fill("textarea", sub.code))
              .then(() => browser.pressButton('input[value="Send"]', next));
          },
        ],
        (err) => {
          let html = browser.html() || "";
